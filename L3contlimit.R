...
for (i in seq(1, length(xis))) {
    ...
    if (fix %in% names(opt)) {
        ...
        if (as.logical(opt[[fix]])) {
            ...
            readin <- readinbootstrapsamples(
                beta = data$beta[j], Ns = data$Ns[j],
                Nt = data$Nt[j], xi = data$xi[j], columns = c(1, 1),
                names = c("bsp", "bsxicalc"), filename = filenameres,
                end = end
            )

            tmpplaqsmall <- readRDS(sprintf("%s/../plaqsingleNt%dNs%dbeta%fxi%fbs%d.RData", opt$respath, data$Nt[j], 3, data$beta[j], xis[i], opt$bootsamples))
...
resultslist[["plaqsmall"]][resultslist$nalist, ] <- NA
resultslist[["xiphys"]][resultslist$nalist, ] <- NA

## build ratio of small and large volume
resultslist[["ratio"]] <- resultslist[["plaqsmall"]] / resultslist[["plaqren"]]
result$ratio <- result$psmall / result$p


print(result)

namesave <- sprintf("%s/contlimitL3data%s.csv", opt$respath, endnamewrite)
write.table(result, file = namesave, col.names = TRUE, row.names = FALSE, append = FALSE)

resultslist$githash <- githash
namesave <- sprintf("%s/contlimitL3data%s.RData", opt$respath, endnamewrite)
...
if (TRUE) {
    ...
    bsamplescontlimitratio <- array(rep(NA, 2 * opt$bootsamples * (length(xis))), dim = c(opt$bootsamples, 2 * length(xis)))


    # fill up containers:
    # beta: take continuum limit for beta(xi) as well
    bsamplescontlimit[, seq(1, length(xis))] <- resultslist[["plaqsmall"]]
    bsamplescontlimit[, seq(length(xis) + 1, 2 * length(xis))] <- resultslist[["xiphys"]]^2
    bsamplescontlimitratio[, seq(length(xis) + 1, 2 * length(xis))] <- resultslist[["xiphys"]]^2
    bsamplescontlimitratio[, seq(1, length(xis))] <- resultslist[["ratio"]][, seq(1, length(xis))]
    if (opt$xisingle) {
        bsamplescontlimit[, seq(length(xis) + 1, 2 * length(xis))] <- resultslist[["xiphys"]]
        bsamplescontlimitratio[, seq(length(xis) + 1, 2 * length(xis))] <- resultslist[["xiphys"]]
    }
    ...
    listnames <- paste0(rep(c("plaqsmall", "ratio"), each = 5), seq(1, 5))
    indexlim <- 1
    pdf(sprintf("%s/plotscontlimitL3%scont%d.pdf", opt$respath, endnamewrite, opt$indexfitcontlim), title = "")
            ...
            for (fun in c(fnlin, fnpar, fncub)) {
                if (uplimfit - lowlimfit > i) {
                    ...
                    fitplaq <- try(bootstrap.nlsfit(fun, rep(1, i + 1),
                        x = xirenfit, y = result$psmall, bsamples = bsamplescontlimit,
                    ...
                    if (!inherits(fitplaq, "try-error")) {
                        try(plot(fitplaq,
                            main = sprintf(
                                "continuum limit plaquette L3: %4f + /-%4f, chi = %4f, p = %4f,\ndegree of polynomial:%d\nlowlim fit: %d, uplim fit: %d",
                        ...
                        resultspolynomial <- rbind(
                            ...
                            data.frame(
                                ...
                                type = "plaqsmall", limplot = fitplaq$t0[1], dlimplot = fitplaq$se[1],
                    ...
                    # ratio cont limit
                    err <- apply(bsamplescontlimitratio, 2, sd, na.rm = T)
                    fitratio <- try(bootstrap.nlsfit(fun, rep(0.1, i + 1),
                        x = xirenfit, y = result$ratio, bsamples = bsamplescontlimitratio,
                        mask = maskfitcontlim, na.rm = TRUE,
                        dy = err[1:length(xirenfit)],
                        dx = err[(1:length(xirenfit)) + length(xirenfit)], 
                        maxiter=opt$maxiter
                    ))
                    fitspolynomial[[listnames[5 + i]]] <- fitratio

                    try(plot(fitratio,
                        main = sprintf(
                            "continuum limit ratio: %4f + /-%4f, chi = %4f, p = %4f,\ndegree of polynomial:%d\nlowlim fit: %d, uplim fit: %d",
                            fitratio$t0[1], fitratio$se[1],
                            fitratio$chi / fitratio$dof, fitratio$Qval, i, lowlimfit, uplimfit
                        ),
                        plot.range = c(-0.2, 1.2),
                        ylim = c(0.99, 1.05),
                        xlab = "xi_ren^2", ylab = "P(L=3) / P(L=16)", xaxs = "i", xlim = c(0, 1.05)
                    ))
                    try(plotwitherror(
                        rep = TRUE, col = "red",
                        x = fitratio$x[fitratio$mask],
                        dx = fitratio$dx[fitratio$mask],
                        y = fitratio$y[fitratio$mask],
                        dy = fitratio$dy[fitratio$mask]
                    ))
                    ...
                    try(resultspolynomial <- rbind(
                        resultspolynomial,
                        data.frame(
                            degree = i, lim = tex.catwitherror(fitratio$t0[1],
                                fitratio$se[1],
                                digits = 2, with.dollar = FALSE
                            ),
                            chi = fitratio$chi / fitratio$dof, p = fitratio$Qval,
                            type = "ratio", limplot = fitratio$t0[1], dlimplot = fitratio$se[1],
                            lowlimfit = lowlimfit, uplimfit = uplimfit
                        )
                    ))
                    ...
                    if (inherits(fitratio, "try-error")) {
                        print(paste("There was an error with fitratio for polynomial degree", i))
                    }
                    i <- i + 1
                }
            }
...
namepol <- sprintf("%s/polynomialbetachosenL3%scont%dmaxiter%d.csv", opt$respath, endnamewrite, opt$indexfitcontlim, opt$maxiter)
write.table(resultspolynomial, namepol, col.names = TRUE, row.names = FALSE)
print(resultspolynomial)

namesave <- sprintf("%s/listpolynomialrenormalizationbetachosenL3%scont%dmaxiter%d.RData", opt$respath, endnamewrite, opt$indexfitcontlim, opt$maxiter)
