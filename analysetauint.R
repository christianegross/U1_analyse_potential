...
for (y in seq(1, Ns * opt$fraction, 1)) {
    ...
    tauintdf <- rbind(tauintdf, data.frame(yt = y, spatial = T, l = 1,
            ...
            errorsnaive = as.data.frame(t(errorsnaive))))
    ...
    for (boot.l in c(2, 4, 8, 16, 32, 64, 128, 256)) {
        ...
        tauintdf <- rbind(tauintdf, data.frame(yt = y, spatial = T, l = boot.l,
                times = as.data.frame(t(autotime)), dtimes = as.data.frame(t(dautotime)),
                errors = as.data.frame(t(errors)), derrors = as.data.frame(t(derrors)),
                errorsnaive = as.data.frame(t(errorsnaive))))
    }}
...
for (t in seq(1, Nt * opt$fraction, 1)) {
    ...
    for (boot.l in c(2, 4, 8, 16, 32, 64, 128, 256)) {
        ...
        tauintdf <- rbind(tauintdf, data.frame(yt = t, spatial = F, l = boot.l,
                times = as.data.frame(t(autotime)), dtimes = as.data.frame(t(dautotime)),
                errors = as.data.frame(t(errors)), derrors = as.data.frame(t(derrors)),
                errorsnaive = as.data.frame(t(errorsnaive))))
    }}
