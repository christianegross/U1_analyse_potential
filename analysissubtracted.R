...
option_list <- list(
    ...
    help="points lower than this are not used for determining xi [default %default]"), 
    ...
    help="if true, correlators and effective masses are determined [default %default]"),
...
if(analyse){
filenameforplots <- sprintf("%splotseffectivemass2p1dsubtractedchosenNs%dbeta%fxi%f.pdf", opt$plotpath, Ns, beta, xi)
if(opt$smearing){
    filenameforplots <- sprintf("%splotseffectivemass2p1dsubtractedchosenNs%dbeta%fxi%fnape%dalpha%f.pdf", opt$plotpath, Ns, beta, xi, nape, alpha)
}
pdf(file=filenameforplots, title="")
listfits <- list()
listtauint <- list()
namelistbounds <- sprintf("%stableanalysissubtractedNs%dNt%dbeta%fxi%f.csv", opt$respath, Ns, Nt, beta, xi)
if(opt$smearing){
    namelistbounds <- sprintf("%stableanalysissubtractedNs%dNt%dbeta%fxi%fnape%dalpha%f.csv", opt$respath, Ns, Nt, beta, xi, nape, alpha)
}
if (file.exists(namelistbounds)){    
    listbounds <- read.table(namelistbounds, sep=",", header=TRUE)
} else{
    listbounds <- cbind(data.frame(yt=c(seq(1, Ns/2), seq(1, Ns/2))), data.frame(spacial=c(rep(TRUE, Ns/2), rep(FALSE, Ns/2))), data.frame(lower=c(rep(opt$lowerboundmeff, Ns/2), rep(floor(opt$lowerboundmeff/xi), Ns/2))), data.frame(upper=c(rep(Ns/2-2, Ns/2),rep(Nt/2-2, Ns/2))))
}
print(namelistbounds)
print(listbounds)
negatives <- c()

filename <- sprintf("%sresult2p1d.u1potential.rotated.Nt%d.Ns%d.b%f.xi%f.nape%d.alpha%fcoarsedistance",
...
#~     print(Ns)
    ...
    title<-sprintf("beta=%.2f coarse x=%d skipped %d", beta, x, skip*opt$nsave)
    
    WL <- calcplotWloopnormalspatial(file=filename, skip=skip, Ns=Ns, x=x, bootsamples=bootsamples, title=title, path=opt$respath,
    zerooffset=opt$zerooffset, every=opt$every, nsave=opt$nsave, l=opt$bootl)
...
#~     print(uwerrresults)
    ...
    WL.effmasslist <- deteffmass(WL=WL, t1=t1, t2=t2, yt=x, potential=potential, isspatial=1)
...
#~     print(listresults[[2]])
    ...
    try(plot(WL.effmasslist[[1]], xlab="y/a_s", ylab="Meff", main=sprintf("%s, t1=%d, t2=%d, p=%f", title, t1, t2, WL.effmasslist[[1]]$effmassfit$Qval)))
    try(plot(WL.effmasslist[[1]], xlab="y/a_s", ylab="Meff", main=sprintf("%s, t1=%d, t2=%d, p=%f", title, t1, t2, WL.effmasslist[[1]]$effmassfit$Qval), ylim=WL.effmasslist[[2]]))
}
...
#~     message("\nx=", x)
    ...
    title<-sprintf("beta=%.2f fine x=%d skipped %d", beta, x, skip*opt$nsave)
    WL <- calcplotWloopnormaltemporal(file=filename, skip=skip, Ns=Ns, Nt=Nt, x=x, bootsamples=bootsamples, title=title, path=opt$respath,
    zerooffset=opt$zerooffset, every=opt$every, nsave=opt$nsave, l=opt$bootl)
    uwerrresults <- uwerr.cf(WL)
#~     print(uwerrresults)
    ...
    WL.effmasslist <- deteffmass(WL=WL, t1=t1, t2=t2, yt=x, potential=potential, isspatial=0)
    #plot results, save results off meff in short and long form (with and without bootstrapsamples
    if(WL.effmasslist[[2]][[2]]!=0){
        listresults <- list(WL.effmasslist[[1]], x, TRUE)
    }
#~     print(listresults[[2]])
    ...
    try(plot(WL.effmasslist[[1]], xlab="t/a_t", ylab="Meff", main=sprintf("%s, t1=%d, t2=%d, p=%f", title, t1, t2, WL.effmasslist[[1]]$effmassfit$Qval)))
    try(plot(WL.effmasslist[[1]], xlab="t/a_t", ylab="Meff", main=sprintf("%s, t1=%d, t2=%d, p=%f", title, t1, t2, WL.effmasslist[[1]]$effmassfit$Qval), ylim=WL.effmasslist[[2]]))
}
...
filenamepotential <- sprintf("%spotentialmeff2p1dsubtractedchosenNs%dbeta%fxi%fbsamples%d.csv", opt$plotpath, Ns, beta, xi, bootsamples)
filenamelist <- sprintf("%slistmeff2p1dsubtractedchosenNs%dbeta%fxi%fbsamples%d.RData", opt$plotpath, Ns, beta, xi, bootsamples)
filenameuwerr <- sprintf("%slistuwerrtauintsubtractedchosenNs%dbeta%fxi%fbsamples%d.RData", opt$plotpath, Ns, beta, xi, bootsamples)
filenamenegatives <- sprintf("%snegativessubtractedNs%dbeta%fxi%fbsamples%d.csv", opt$plotpath, Ns, beta, xi, bootsamples)

if(opt$smearing){
    filenamepotential <- sprintf("%spotentialmeff2p1dsubtractedchosenNs%dbeta%fxi%fnape%dalpha%fbsamples%d.csv", opt$plotpath, Ns, beta, xi, nape, alpha, bootsamples)
    filenamelist <- sprintf("%slistmeff2p1dsubtractedchosenNs%dbeta%fxi%fnape%dalpha%fbsamples%d.RData", opt$plotpath, Ns, beta, xi, nape, alpha, bootsamples)
    filenameuwerr <- sprintf("%slistuwerrtauintsubtractedchosenNs%dbeta%fxi%fnape%dalpha%fbsamples%d.RData", opt$plotpath, Ns, beta, xi, nape, alpha, bootsamples)
    filenamenegatives <- sprintf("%snegativessubtractedNs%dbeta%fxi%fnape%dalpha%fbsamples%d.csv", opt$plotpath, Ns, beta, xi, nape, alpha, bootsamples)
}
...
write.table(data.frame(x=seq(1, Ns/2), neg=negatives), file=filenamenegatives, row.names=FALSE)
...
filenameforplots <- sprintf("%spotentialmeff2p1dsubtractedchosenNs%dbeta%fxi%fomit%dlowlim%d.pdf", opt$plotpath, Ns, beta, xi, opt$omit, opt$lowlim)
if(opt$smearing){
filenameforplots <- sprintf("%spotentialmeff2p1dsubtractedchosenNs%dbeta%fxi%fnape%dalpha%flowlim%d.pdf", opt$plotpath, Ns, beta, xi, nape, alpha, opt$lowlim)
}
pdf(file=filenameforplots, title="")
...
fnpotscal <- function (par, x, boot.r, finemask, ...) par[1] + par[2] * x + par[3] * log(x) + par[4] * x[finemask]
...
plaquettecf <- readloopfilecfonecolumn(file=filename, skip=skip, column=column)#, every=opt$every, boot.l=opt$bootl)
#~ summary(plaquettecf)
...
filenamecoarse <- sprintf("%sfitresultcoarsesubtractedb%fxi%fNs%dbsamples%domit%dl%d.RData", opt$plotpath, beta, xi, Ns, bootsamples, opt$omit, t1)
if(opt$smearing){
filenamecoarse <- sprintf("%sfitresultcoarsesubtractedb%fxi%fNs%dnape%dalpha%fbsamples%domit%dl%d.RData", opt$plotpath, beta, xi, Ns, nape, alpha, bootsamples, opt$omit, t1)
}
...
filenamefine <- sprintf("%sfitresultfinesubtractedb%fxi%fNs%dbsamples%domit%dl%d.RData", opt$plotpath, beta, xi, Ns, bootsamples, opt$omit, t1)
if(opt$smearing){
filenamefine <- sprintf("%sfitresultfinesubtractedb%fxi%fNs%dnape%dalpha%fbsamples%domit%dl%d.RData", opt$plotpath, beta, xi, Ns, nape, alpha, bootsamples, opt$omit, t1)
}
...
plot(fit.resultfine, main = sprintf("beta=%f, xi=%f, (2+1)D, Ns=%d, fine\n%d measurements of which skipped %d", beta, xi, Ns, nom*100, skip*100), ylab="a_t V_{xt}(x)", xlab="x/a_s")
plot(fit.resultcoarse, main = sprintf("beta=%f, xi=%f, (2+1)D, Ns=%d, coarse\n%d measurements of which skipped %d", beta, xi, Ns, nom*100, skip*100), ylab="a_s V_{xy}(x)", xlab="x/a_s")
...
#~         for (r2 in seq(r1+1, Ns/2)){
        for (r2 in seq(r1+1, opt$rmax)){
            ...
            xicalc <- ( num / den )
            xilist <- rbind(xilist, data.frame(xi=xicalc, rdiff=abs(r1-r2), r1=r1))
        }
}
}
xilist <- na.omit(xilist)
...
fit.match <- bootstrap.nlsfit(matchpot, c(0.1, xi), y=yc, x=yf, bsamples[,1:Ns], mask=maskc)
#~ fit.match <- bootstrap.nlsfit(matchpot, c(0.1, xi), x=yc, y=yf, bsamplesmatch)
plot(fit.match, xlab="a_tV_s(x)", ylab="a_sV_s(x)", main="matching potentials to determine xi")
print(fit.match)
...
filenamematch <- sprintf("%sfitresultmatchsubtractedb%fxi%fNs%dbsamples%domit%dl%dlowlim%d.RData", opt$plotpath, beta, xi, Ns, bootsamples, opt$omit, t1, opt$lowlim)
if(opt$smearing){
...
filenamematch <- sprintf("%sfitresultmatchsubtractedb%fxi%fNs%dnape%dalpha%fbsamples%domit%dl%dlowlim%d.RData", opt$plotpath, beta, xi, Ns, nape, alpha, bootsamples, opt$omit, t1, opt$lowlim)
}
...
fit.resultscaled <- bootstrap.nlsfit(fnpot, c(0.1, 0.1, 0.1), y, x, bsamples, mask=mask)
filenamescaled <- sprintf("%sfitresultscaledsubtractedb%fxi%fNs%dbsamples%domit%dl%dlowlim%d.RData", opt$plotpath, beta, xi, Ns, bootsamples, opt$omit, t1, opt$lowlim)
if(opt$smearing){
filenamescaled <- sprintf("%sfitresultscaledsubtractedb%fxi%fNs%dnape%dalpha%fbsamples%domit%dl%dlowlim%d.RData", opt$plotpath, beta, xi, Ns, nape, alpha, bootsamples, opt$omit, t1, opt$lowlim)
}
#~ print(fit.resultscaled)
...
plot(fit.resultscaled, main = title, ylab="V(r)", xlab="r/a_s", pch=3, cex=0.1)
pointsyerr(x=fit.resultscaled$x[finemask], y=fit.resultscaled$y[finemask], dy=fit.resultscaled$dy[finemask], col=1, pch=1)
pointsyerr(x=fit.resultscaled$x[!finemask], y=fit.resultscaled$y[!finemask], dy=fit.resultscaled$dy[!finemask], col=2, pch=2)
legend(legend=c("spatial", "temporal"), col=c(2,1), pch=c(2,1), x="topleft")


#force with error
plot(xx, fnforce(fit.resultscaled$t0, xx, 0), type="l", xlab="r/a_s", ylab="-r^2 d/dr V(r)", main = title, xlim=c(0,8))
...
polygon(x=c(xx, rev(xx)), y=polyval, col=pcol, lty=0, lwd=0.001, border=pcol)
...
plot(xx, fnforce(fit.resultscaled$t0, xx, 0), type="l", xlab="r/a_s", ylab="-r^2 d/dr V(r)", main = title, xlim=xlim, ylim=ylim)
polygon(x=c(xx, rev(xx)), y=polyval, col=pcol, lty=0, lwd=0.001, border=pcol)
...
#arrows(xhi, yhi, xlo, ylo)
    arrows(rzero, fnforce(fit.resultscaled$t0, rzero, 0), rzero, lowylim, angle=90, length=0.1, code=0)
    ...
    polygon(x=r0polyval2, y=r0polyval1 , col=pcol, lty=0, lwd=0.001, border=pcol)
}
...
#plot thermalisation
plot(plaquettecolumn, main="Thermalisation", xlab=sprintf("MCMC-steps/%d", nsave), ylab="P")
...
resultlist <- data.frame(xi=NA, beta=NA, xicalc=NA, dxicalc=NA, xi2=NA,
                         dxi2=NA, r0=NA, dr0=NA, st=NA, dst=NA, p=NA, dp=NA,
                         chi=NA, diff=NA, ddiff=NA, bs=NA,
                         xicalcsub=NA, dxicalcsub=NA, xi2sub=NA, dxi2=NA,
                         c=NA, rmin=NA, rmax=NA, Ns=NA, Nt=NA, 
                         nape=NA, alpha=NA, omit=NA, nom=NA, skip=NA, t1=NA, job=NA,
...
newline <- data.frame(xi=xi, beta=beta, xicalc=xicalc, dxicalc=dxicalc,
                      ...
                      diff=fit.match$t0[1], ddiff=fit.match$se[1], bs=bootsamples,
                      xicalcsub=xicalcsub, dxicalcsub=dxicalcsub, xi2sub=xisquaredsub, dxi2=dxisquaredsub,
                      c=rzeroofc$c[i], rmin=opt$rmin, rmax=opt$rmax,
                      Ns=Ns, Nt=Nt, nape=nape, alpha=alpha, omit=opt$omit, nom=nom, skip=skip,
                      t1=t1, job=opt$job, every=opt$every,
                      tauint=plaquettedata$tauint, dtauint=plaquettedata$dtauint, bootl=opt$bootl, lowlim=opt$lowlim)
resultlist <- rbind(resultlist, newline)
}

resultlist <- resultlist[-1, ]
print(resultlist)
filename <- sprintf("%sresultsummary2p1dnormalb%.3fNs%d.csv", opt$plotpath, opt$betaone, opt$Ns)
...
nameresults <- sprintf("%sresultssubtractedNs%dNt%dbeta%fxi%fbs%d.RData", opt$plotpath, Ns, Nt, beta, xi, bootsamples)
...
listmeff <- readRDS(file=filenamelist)
  ...
  filenamelist <- sprintf("%slistmeff2p1dsubtractedNs%dbeta%fxi%fbsamples%dl%d.RData", opt$plotpath, Ns, beta, xi, bootsamples, t1)
if(opt$smearing){
    filenamelist <- sprintf("%slistmeff2p1dsubtractedNs%dbeta%fxi%fnape%dalpha%fbsamples%dl%d.RData", opt$plotpath, Ns, beta, xi, nape, alpha, bootsamples, t1)
}
  listmeff <- readRDS(file=filenamelist)
}

filenameforplots <- sprintf("%splotsonlymeff2p1dsubtractedchosenNs%dbeta%fxi%f.pdf", opt$plotpath, Ns, beta, xi)
if(opt$smearing){
    filenameforplots <- sprintf("%splotsonlymeff2p1subtractedchosenNs%dbeta%fxi%fnape%dalpha%f.pdf", opt$plotpath, Ns, beta, xi, nape, alpha)
}
pdf(file=filenameforplots, title="")
...
for(i in seq(1,Ns)){
    ...
    if(i<=Ns/2){
        if(!is.null(listmeff[[i]][[1]]$effmassfit)){
            title <- sprintf("%s coarse, x=%d, p=%f", titleall, i, listmeff[[i]][[1]]$effmassfit$Qval)
        } else{
...
#~         print(names(listmeff[[i]][[1]]))
        ...
        ylim <- c(min, max)
        message(ylim)
        try(plot(listmeff[[i]][[1]], xlab="y/a_s", ylab="Meff", main=title))
        try(plot(listmeff[[i]][[1]], xlab="y/a_s", ylab="Meff", ylim=ylim, main=title))
    }
    if(i>Ns/2){
        if(!is.null(listmeff[[i]][[1]]$effmassfit)){
            title <- sprintf("%s fine, x=%d, p=%f", titleall, i-Ns/2, listmeff[[i]][[1]]$effmassfit$Qval)
        } else{
...
#~         message("blubb")
        ...
        ylim <- c(min, max)
        message(ylim)
        try(plot(listmeff[[i]][[1]], xlab="t/a_t", ylab="Meff", main=title))
        try(plot(listmeff[[i]][[1]], xlab="t/a_t", ylab="Meff", ylim=ylim, main=title))
    }    
    }
}
}
