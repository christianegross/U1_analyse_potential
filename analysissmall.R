...
option_list <- list(
    ...
    help="if true, potential is determined through effective mass [default %default]"),
...
if(opt$analyse){
namelistbounds <- sprintf("%stableanalysissmallNs%dNt%dbeta%fxi%f.csv", opt$plotpath, Ns, Nt, beta, xi)
if(opt$smearing){
    namelistbounds <- sprintf("%stableanalysissmallNs%dNt%dbeta%fxi%fnape%dalpha%f.csv", opt$plotpath, Ns, Nt, beta, xi, nape, alpha)
}
...
if (file.exists(namelistbounds)){    
    ...
    listbounds <- cbind(data.frame(x=c(rep(0,4), rep(1,4), rep(2,4), rep(3,4))), data.frame(y=rep(c(0,1,2,3), 4)), data.frame(lower=rep(opt$t1, 16)), data.frame(upper=rep(Ntmax-opt$t1-2, 16)))
}
    

print(listbounds)
filenameforplots <- sprintf("%splots2p1dsmallNs%dNt%dbeta%fxi%fbs%d.pdf", opt$plotpath, Ns, Nt, beta, xi, bootsamples)
if(opt$smearing){
    filenameforplots <- sprintf("%splots2p1dsmallNs%dNt%dbeta%fxi%fnape%dalpha%fbs%d.pdf", opt$plotpath, Ns, Nt, beta, xi, nape, alpha, bootsamples)
}
...
for (x in seq(0, 3)){
    ...
    #save results, list has to have an entry everywhere
        ...
        uwerrresults <- FALSE
        message("\nx=", x, " y=", y)
        filename <- sprintf("%sresult2p1d.u1potential.Nt%d.Ns%d.b%f.xi%f.nape%d.alpha%fnonplanar", opt$respath ,Nt, Ns, beta, xi, nape, alpha)
    ...
    #~     if(!wlexist){
        if(y==0 || x==0){
            start <- 1
            title <- sprintf("beta=%.3f x=%d  y=%d W(t=0) not used\n", beta, x, y)
        } else{ 
            start <- 0
            title <- sprintf("beta=%.3f x=%d  y=%d W(t=0) used\n", beta, x, y)
        }
        WL <- calcplotWloopsmall(filename, skip, Nsmax, Ntmax, Nt, x, y, start, bootsamples, title, nsave=opt$nsave)
        
        title <- sprintf("%s%d configs, skipped %d",title, (length(WL$cf[,1])+skip-1)*opt$nsave, skip*opt$nsave)
...
#~         listresults <- resulteffmass[[3]]
        ...
        try(plot(resulteffmass[[3]][[1]], xlab="t/a_t", ylab="Meff", main=sprintf("%s, t1=%d, t2=%d, p=%f", title, t1, t2, resulteffmass[[3]][[1]]$effmassfit$Qval)))
        arrows((Nt-start-1)/2, 10, (Nt-start-1)/2, -10, angle=90, length=0.1, code=0, col=2)
        drawallticks()
        try(plot(resulteffmass[[3]][[1]], xlab="t/a_t", ylab="Meff", main=sprintf("%s, t1=%d, t2=%d, p=%f", title, t1, t2, resulteffmass[[3]][[1]]$effmassfit$Qval), ylim=resulteffmass[[2]]))
        arrows((Nt-start-1)/2, 10, (Nt-start-1)/2, -10, angle=90, length=0.1, code=0, col=2)
...
#~         try(plot(WL.effmass, xlab="t/a_t", ylab="Meff", main=sprintf("%s, t1=%d, t2=%d, p=%f", title, t1, t2, WL.effmass$effmassfit$Qval), ylim=c(0,1)))
        ...
        try(plot(x=uwerrresults$uwcf$t-1, y=uwerrresults$uwcf$tauint, main=sprintf("%s, autocorellation times", title), xlab="t", ylab="tauint"))
    }
    }
}
#write out results
filenamelist <- sprintf("%slistmeffsmallNs%dNt%dbeta%fxi%fbsamples%d.RData", opt$plotpath, Ns, Nt, beta, xi, bootsamples)
filenameuwerr <- sprintf("%slistuwerrtauintsmallNs%dNt%dbeta%fxi%fbsamples%d.RData", opt$plotpath, Ns, Nt, beta, xi, bootsamples)
filenamepot <- sprintf("%slistpotsmallNs%dNt%dbeta%fxi%fbsamples%d.csv", opt$plotpath, Ns, Nt, beta, xi, bootsamples)

if(opt$smearing){
    filenamelist <- sprintf("%slistmeffsmallNs%dNt%dbeta%fxi%fnape%dalpha%fbsamples%d.RData", opt$plotpath, Ns, Nt, beta, xi, nape, alpha, bootsamples)
    filenameuwerr <- sprintf("%slistuwerrtauintsmallNs%dNt%dbeta%fxi%fnape%dalpha%fbsamples%d.RData", opt$plotpath, Ns, Nt, beta, xi, nape, alpha, bootsamples)
    filenamepot <- sprintf("%slistpotsmallNs%dNt%dbeta%fxi%fnape%dalpha%fbsamples%d.csv", opt$plotpath, Ns, Nt, beta, xi, nape, alpha, bootsamples)
}
...
if(opt$plaquette){
    filenameforplots <- sprintf("%spotential2p1dsmallplaqNs%dNt%dbeta%fxi%fbs%d.pdf", opt$plotpath, Ns, Nt, beta, xi, opt$bootsamples)
    if(opt$smearing){
        filenameforplots <- sprintf("%spotential2p1dsmallplaqNs%dNt%dbeta%fxi%fnape%dalpha%f.pdf", opt$plotpath, Ns, Nt, beta, xi, nape, alpha)
    }
    pdf(filenameforplots, title="")    
    if(opt$bootsamples==0){
        bootsamples <- find_bootsamples(Nt=Nt, Ns=Ns, xi=xi, beta=beta, plotpath=opt$plotpath)
    }
    ...
    if(TRUE){ #plaquette
        plaquettedata <- readloopfilecfplaquette(file=filename, skip=skip, Nsmax=Nsmax, Ntmax=Ntmax)
...
#~         print(listmeff[[5]][[3]])
    ...
    if(listmeff[[4]][[2]] && listmeff[[5]][[2]]){
        ...
        fit.result <- try(bootstrap.nlsfit(fnlin, c(0.2, 0.2), pot, x, bsamplespot))
    ...
    title <- sprintf("beta=%f, xi_in=%f, Ns=%d, Nt=%d", beta, xi, Ns, Nt)
    error3 <- try(plot(fit.result, xlab="r/a_s", ylab="a_tV(r)", main=title))
    print(error3)
    if(inherits(error3, "try-error")) try(plotyerr(x=x, y=pot, dy=dpot, xlab="r/a_s", ylab="a_tV(r)", main=title))
...
#~         print(V)
        ...
        fit.pot <- try(bootstrap.nlsfit(fnpot, c(1,1,1), y=V, x=sqrt(x^2+y^2), bsamples, mask=mask))
        if(!inherits(fit.pot, "try-error")){
            print(fit.pot)
            plot(fit.pot, ylab="a_tV(r)", xlab="r/a_s", main="Potential including nonplanar points", plot.range=c(1e-5, sqrt(18)), xlim=c(0, sqrt(18)))
            st <- fit.pot$t0[2]
            dst <- fit.pot$se[2] 
            bsst <- fit.pot$t[, 2]
            icpot <- fit.pot$t0[1]
            ...
            for(i in seq(1, length(crz))){
                rzerovec <- determinerzero(fit.pot, c=crz[i], xi=xi, bootsamples=bootsamples)
                rzeros[i] <- rzerovec[[1]]
            ...
            plotyerr(x=sqrt(x^2+y^2), y=V, dy=apply(bsamples, 2, sd), ylab="a_tV(r)", xlab="r", main="Potential including nonplanar points")
            st <- NA
            dst <- NA
    ...
    results <- data.frame(beta=NA, xiin=NA, Nt=NA, Ns=NA, p=NA, dp=NA,
     slope=NA, dslope=NA, chislope=NA,
...
#~          " a ", plaquetteuwerr, " a ", dpuwerr, " a ", opt$job, " a ", bootsamples)
        newline <- data.frame(beta=opt$beta, xiin=xi, Nt=opt$Nt, Ns=opt$Ns, p=plaquette,
         dp=dp, slope=slope, dslope=dslope, chislope=chislope,
         ratio=ratio, dratio=dratio, st=st, dst=dst, chipot=chipot,
         icslope=icslope, dicslope=dicslope, icpot=icpot, dicpot=dicpot, logpot=logpot, dlogpot=dlogpot,
         ratioslope=ratioslope, dratioslope=dratioslope, ratiopot=ratiopot, dratiopot=dratiopot,
...
#~         print(results)
    ...
    results <- results[-1, ]
    print(results)
    filenamexi <- sprintf("%ssummarysmallbetaone%f.csv", opt$plotpath, opt$betaone)
    columnnames <- FALSE
    if(!file.exists(filenamexi)){
        columnnames <- TRUE
    }
    write.table(results, filenamexi, append=TRUE, row.names=FALSE, col.names=columnnames)
    nameresults <- sprintf("%sresultspotNs%dNt%dbeta%fxi%fbs%d.RData", opt$plotpath, Ns, Nt, beta, xi, bootsamples)
    saveRDS(resultssummary, file=nameresults)
    
    filenamefineinteger <- sprintf("%sfitresultfinesubtractedb%fxi%fNs%dbsamples%domit0l0.RData", opt$plotpath, beta, xi, Ns, bootsamples)
    if(opt$smearing){
        filenamefineinteger <- sprintf("%sfitresultfinesubtractedb%fxi%fNs%dnape%dalpha%fbsamples%domit0l0.RData", opt$plotpath, beta, xi, Ns, nape, alpha, bootsamples)
    }
#~     print(filenamefineinteger)
    resultfineinteger <- try(readRDS(filenamefineinteger))
#~     print(resultfineinteger)
    try(plot(fit.pot, ylab="a_tV(r)", xlab="r/a_s", main="Potential including nonplanar points"))
    try(errorpolygon(X=seq(0, 7, by=0.05), resultfineinteger, col.p=2, pch=2))
    try(legend(x="topleft",legend=c("non-integer", "integer"), col=c(1,2), pch=c(1, 2)))
    
    try(plot(resultfineinteger, ylab="a_tV(r)", xlab="r/a_s", main="Potential including nonplanar points", col=2, pch=2))
