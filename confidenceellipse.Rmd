...
```{r}
draw_ellipse <- function(x, y, meanx=mean(x), meany=mean(y), points=1000, verbose=TRUE, nstd=1, ...) {
  ...
  plotwitherror(x=meanx, y=meany, dx=sd(x), dy=sd(y), rep=TRUE)
  polygon(x=c(meanx - sd(x), meanx + sd(x), meanx + sd(x), meanx - sd(x)), 
