...
fitresults <- data.frame(xiin=NA, r0slope=NA, r0intercept=NA, chir0=NA, pr0=NA, plaqslope=NA, plaqintercept=NA, chiplaq=NA, pplaq=NA)
...
print(length(newframe))
print(length(fitresults[,1]))
...
#~     print(summary(fitplaqnaive))
    ...
    try(plot(fitplaqnaive, main=sprintf("cont. lim. plaq.: %f+/-%f, chi=%f,\ndegree of polynomial:%d", fitplaqnaive$t0[1],
            fitplaqnaive$se[1] ,fitplaqnaive$chi/fitplaqnaive$dof, i), plot.range=c(-0.2, 1.2), ylim=c((fitplaqnaive$t0[1]-fitplaqnaive$se[1]),
            max(result$p)), xlab="", xaxs="i", xlim=c(0,1)))
    ...
    fitplaq <- try(bootstrap.nlsfit(fun, rep(1, i+1), x=xis[c(1, 2, 3, 4, 5, 6, 8)]^2, y=result$p, bsamples=bsamplescontlimit))
    fitspolynomial[[5+i]] <- fitplaq
    if (!inherits(fitplaq, "try-error")){
#~     print(summary(fitplaq))
#~     print(fitplaq$x)
#~     print(fitplaq$y)
#~     plot(fitplaq, main=sprintf("cont. lim. plaq.: %f+/-%f, chi=%f, p=%f,\ndegree of polynomial:%d", fitplaq$t0[1], fitplaq$se[1] ,fitplaq$chi/fitplaq$dof, fitplaq$Qval, i), plot.range=c(-0.2, 1.2), ylim=c((fitplaq$t0[1]-fitplaq$se[1]), max(result$p)), xlab="", xaxs="i", xlim=c(0,1))
    plot(fitplaq, main=sprintf("cont. lim. plaq.: %f+/-%f, chi=%f, p=%f,\ndegree of polynomial:%d", fitplaq$t0[1],
            fitplaq$se[1] ,fitplaq$chi/fitplaq$dof, fitplaq$Qval, i), plot.range=c(-0.2, 1.2), xlab="", xaxs="i")
...
write.table(result, sprintf("plotstikz/resultsrenormalizationnaiveomit%d.csv", omit), col.names=TRUE, row.names=FALSE, append=FALSE)
write.table(fitresults, sprintf("plotstikz/fitresultsrenormalizationnaiveomit%d.csv", omit), col.names=TRUE, row.names=FALSE, append=FALSE)
